...
public abstract class ConsoleActivityM2 extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(resId("layout", "activity_consolem2"));
        createInput();
        createOutput();
        ...
        task.inputEnabled.observe(this, new Observer<Boolean>() {
            @Override public void onChanged(@Nullable Boolean enabled) {
                ...
                if (enabled) {
                    ...
                    tvOutput.setSelection(tvOutput.getText().length());
    ...
    private void output(CharSequence text) {
        ...
        if (scrollback.length() > MAX_SCROLLBACK_LEN) {
            scrollback.delete(0, MAX_SCROLLBACK_LEN / 5);
        }
        ...
        public void start() {
            new Thread(() -> {
                try {
                    ...
                    //Add code for btn try again visible and others invisible
                } finally {
